...
class TeamFeedPageLocators:
     ...
     TEST_REVIEW_XPATH = "//div[text()='팀피드실시간테스트']"
     ...
     GROUP_DINNER_XPATH = "//label[@for='그룹']"  
     TEAM_DINNER_XPATH = "//label[@for='회식']" 
     ...
     image_path = os.path.abspath(r"C:\\Users\\LENOVO\\OneDrive\\바탕 화면\\qa-track-ch6-reference-main\\tests\\utils\\깜자.png")
     ...
     review_star_yellow = ".//div[contains(@class, 'text-yellow-400')]"
     ...
     # 팀의 맛 성향 관련 선택자
     TEAM_SWEET_FLAVOR_XPATH = "//section[span[text()='단 맛']]//span[@role='slider']"  # 단 맛 성향
     TEAM_SALTY_FLAVOR_XPATH = "//section[span[text()='짠 맛']]//span[@role='slider']"  # 짠 맛 성향
     TEAM_SPICY_FLAVOR_XPATH = "//section[span[text()='매운 맛']]//span[@role='slider']"  # 매운 맛 성향
     TEAM_FLAVOR_REQUIRED_MESSAGE_XPATH = "//p[contains(text(), '맛에 대한 성향은 최소 1 이상 설정해주세요')]"  # 별점 필수 메시지
     SLIDER_BAR_CSS = 'span[data-orientation="horizontal"]'
     SLIDER_VALUE_SPAN_CSS = 'span.w-8.text-right.text-gray-500.text-subbody'
     ...
     SAME_MENU_IMAGE_CSS = "img.object-cover.w-full.h-full.rounded-md.border-\\[1px\\].border-light-gray"
     ...
     SAME_MENU_NAME_CSS = "input.flex.w-full.rounded-md[name='menu']"
     SAME_MENU_CATEGORY_XPATH = "button[role='combobox'] span"
     SAME_MENU_REVIEW_XPATH = "//textarea[@name='comment']"
     SAME_MENU_REVIEW_TEXT = "이번에 부장님을 모셨는데 비린 맛이 난다고 싫어하시더라구요 여긴 다시 안 가야겠습니다"
     NEW_REVIEW_TEXT = "가"*30
     SAME_MENU_STAR_CSS = "input[name='star']"
